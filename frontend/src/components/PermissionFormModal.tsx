...
export type PermissionFormMode =
  | { kind: "add" }
  | { kind: "edit"; permission: PermissionListItem };

interface Props {
  open: boolean;
  mode: PermissionFormMode;
  onClose: () => void;
  onSaved: (item: PermissionListItem, mode: PermissionFormMode["kind"]) => void;
}
...
export default function PermissionFormModal({ open, mode, onClose, onSaved }: Props) {
  ...
  useEffect(() => {
    if (!open) return;
    if (mode.kind === "edit") {
      setName(mode.permission.name);
      setDescription(mode.permission.description ?? "");
    } else {
      setName("");
      setDescription("");
    }
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (mode.kind === "add") {
        ...
        const updated = await permissionsApi.update(mode.permission.id, body);
      ...
      <form onSubmit={onSubmit} className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder="e.g. transportation.view"
