    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="inline-flex gap-1.5">
          <Can do="roles.edit">
            <IconButton tooltip="Edit role" icon={<PencilIcon />} onClick={() => setModalMode({ kind: "edit", role: row.original })} />
          </Can>
          <Can do="roles.delete">
            <IconButton tooltip="Delete role" tone="danger" icon={<TrashIcon />} onClick={() => setPendingDelete(row.original)} />
          </Can>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-semibold text-slate-800">{t.pages.roles.title}</h2>
        <div className="flex gap-2">
          ...
          <Can do="roles.add">
            <button onClick={() => setModalMode({ kind: "add" })} className="px-3 py-2 text-sm rounded bg-blue-600 hover:bg-blue-700 text-white font-medium flex items-center gap-1.5">
              ...
              {t.pages.roles.addButton}
            </button>
          </Can>
        </div>
      </div>
      {error && <div className="rounded bg-red-50 text-red-700 text-sm p-3 border border-red-200">{error}</div>}
      <DataTable table={table} loading={loading} emptyMessage={t.pages.roles.noData} />
      <RoleFormModal open={modalMode !== null} mode={modalMode ?? { kind: "add" }} onClose={() => setModalMode(null)} onSaved={onSaved} />
      <ConfirmDialog
        open={pendingDelete !== null}
        title={t.modal.roles.deleteTitle}
        message={pendingDelete ? t.modal.roles.deleteMessage.replace("{{name}}", pendingDelete.name) : ""}
