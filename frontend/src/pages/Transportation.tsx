...
import { transportationsApi, usersApi, vehiclesApi, getStoredUser } from "../services/api";
...
export default function Transportation() {
  ...
  // Load driver and vehicle lookup options on mount
  useEffect(() => {
    Promise.all([
      usersApi.drivers().catch(() => []),
      vehiclesApi.list().catch(() => []),
    ])
      .then(([drivers, vehicleList]) => {
        const isDriverUser = (drivers ?? []).some((d) => d.id === currentUserId);
        setIsDriver(isDriverUser);

        const dOpts: [number, string][] = (drivers ?? []).map((d) => [
          d.id,
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await transportationsApi.list({
        pageNumber: pagination.pageIndex + 1,
        pageSize: pagination.pageSize,
        transportedById: isDriver
          ? currentUserId
          : filterDriverId !== ""
          ? filterDriverId
          : undefined,
  ...
  function onSaved(_item: TransportationListItem, kind: TransportationFormMode["kind"]) {
    if (kind === "add") {
    ...
    () => [
      {
        ...
        cell: ({ row }) => (
          <span className="font-medium text-slate-800">{row.original.transportedByName}</span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.noOfTip != null ? (
            String(row.original.noOfTip)
          ) : (
      ...
      },
      {
        ...
        cell: ({ row }) =>
          row.original.materialCost != null ? (
      ...
      },
      {
        ...
        cell: ({ row }) => {
          const total =
            row.original.totalWages ??
            (row.original.wages
              ? row.original.wages * Math.max(1, row.original.noOfTip ?? 1)
              : null);
          return total != null ? (
            `${t.common.currencySymbol} ${total.toLocaleString(undefined, {
          ...
          <div className="inline-flex gap-1.5">
            <Can do="transportation.edit">
              <IconButton
                tooltip="Edit"
                icon={<PencilIcon />}
                onClick={() => openModal({ kind: "edit", transportation: row.original })}
              />
            </Can>
  ...
  return (
    <div className="space-y-5">
      ...
      <DataTable
        table={table}
        loading={loading}
        emptyMessage={t.pages.transportation.noData}
